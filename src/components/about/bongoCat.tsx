...
const BongoCat = () => {
  ...
  // terminal animation
  const terminalAnimationRef = useRef<gsap.core.Timeline | null>(null);
  useEffect(() => {
    if (!containerRef.current) return;

    // Terminal code animation
    terminalAnimationRef.current = gsap.timeline().fromTo(
      ".terminal-code line",
    ...
    // Cleanup function
    return () => {
      if (terminalAnimationRef.current) {
        terminalAnimationRef.current.kill();
      }
    };
  }, []);
  ...
  return (
    ...
    >
      <svg
        ...
        className="overflow-visible w-full h-72"
      >
        <g
          id="bongo-cat"
          className="dark:fill-backgroundCat fill-white stroke-4 stroke-linecap: round stroke-linejoin: round"
        >
          <g className="head stroke-orange">
            ...
            <g className="headphone headphone-right fill-none">
              <g className="speaker">
                <path
          ...
          <polygon
            className="laptop-base stroke-pink"
            points="103.2 263.6 258.9 219.3 636.5 294.4 452.1 339 103.2 263.6"
          ></polygon>
          <g className="laptop-keyboard stroke-blue stroke-2">
          ...
          <polygon
            className="terminal-frame stroke-blue"
            points="93.8 63.3 284.1 73 335.9 230.5 146.2 197.6 93.8 63.3"
          ></polygon>
          <g className="terminal-code stroke-cyan stroke-5">
          ...
          <polygon
            className="laptop-cover fill-none stroke-pink"
            points="103.2 263.6 452.1 339 360.8 12.4 2 2 103.2 263.6"
          ></polygon>
          <g className="paw paw-left  stroke-orange">
            <g
              className={`up inset-0 transition-opacity duration-75 ${
                !showPaw ? "opacity-100" : "opacity-0"
              }`}
          ...
          <g className="headphone headphone-left fill-none">
            <g className="speaker">
              <path
                className="stroke-cyan"
